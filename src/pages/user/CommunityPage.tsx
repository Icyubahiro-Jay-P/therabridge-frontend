...
export function CommunityPage() {
  ...
  useEffect(() => {
    ...
    function handleVisibility() {
      if (document.hidden && screenshotOverlayRef.current) {
        screenshotOverlayRef.current.classList.remove("hidden")
      } else if (screenshotOverlayRef.current) {
  ...
  // Resolve inviteKey from URL -> fetch by key
  useEffect(() => {
    if (!inviteKey) {
      setActive(null)
      setMessages([])
    ...
    if (found) {
      ...
      // Fetch directly by key (e.g. on direct URL entry)
      let mounted = true
